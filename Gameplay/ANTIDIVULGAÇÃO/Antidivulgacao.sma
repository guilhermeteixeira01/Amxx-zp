...
{ 
	"NF | Jogador Novo",
	"NF | Noob ",
	"NF | Instalei o CS agora",
	"NF | Player Noob",
	"NF | Nao sei mudar de nick ",
	"NF | Peguei o CS da China ",
	"NF | CS do Paraguai ",
	"NF | PC nao roda CS:GO ",
	"NF | SEMNOME ",
	"NF | CS Piratinha ",
	"NF | Non-Steam Noob ",
	"NF | Minha Placa nao eh Opengl ",
	"NF | Topo Tudo ",
	"NF | 16bpp ",
	"NF | Botei o CS com Baidu junto ",
	"NF | Palmeiras 100 Mundial ",
	"NF | Obeso ",
	"NF | Ixkill van helsing ",
	"NF | Comi mae do adm ",
	"NF | Dust 2 ",
	"NF | Joga um pouco no ZM ",
	"NF | Cancela Noobs ",
	"NF | Minecrafeteiro ",
	"NF | Fortinite ",
	"NF | Freefirezeiro ",
	...
	"NF | Net de Bosta ",
	"NF | 10 de FPS ",
	"NF | Sou o Hit de tao lagadao ",
	"NF | Sonic eh melhor q Mario ",
	"NF | Software Mode ",
	"NF | Pica de mel ",
	"NF | Leiteiro da aliexpress ",
	"NF | Carteiro ",
	"NF | PaulaTejano ",
	"NF | LucasMonstrao ",
	"NF | Que issu lek ",
	"NF | Ultra Vibe de talco ",
	"NF | 20TeMatar70Correr ",
	"NF | Slendao ",
	"NF | Xandao ",
	"NF | Askonzao ",
	"NF | MaradonaPRO ",
...
{  
    ...
    // Vars
	pCvar_MaxDigits = register_cvar( "antidivulgacao_maxdigito", "10")
	...
	// Format
	format(cFile, charsmax(cFile), "%s/rezombie/AntiDivulgacao.ini", cFile);	
...
public CmdSay(id) 
{  
	if(!(get_user_flags(id) & read_flags("lm")))
	{
	 	// Read & Remove
...
public ChangeNick(id)
{
    ...
    {		
       	// Remover Tag
		formatex(NewName, charsmax(NewName), "name ^"NF | IP: 131.196.198.52:27342^"");
...
public AntiReklamaLog(id, szMessage[])
{
	...
	// Log File
	log_to_file("NF_antidivulgacao.log", szLogData)
...
{             
	...
	len += format(motd[len], charsmax(motd) - len,"<h4 align='center'><font size='6' color='#00FF00'>[NF] Anti Divulgacao^n</font>")
	...
	len += format(motd[len], charsmax(motd) - len,"<h4 align='center'><font size='3' color='#00FF00'><h4>Att: Staff NF</h4></font>")
	len += format(motd[len], charsmax(motd) - len,"<h4 align='center'><font size='3' color='#00FF00'><h4>Duvidas: (61) 99964-7021</h4></font>")
	len += format(motd[len], charsmax(motd) - len,"<h4 align='center'><font size='3' color='#00FF00'><h4>Discord: discord.gg/nightfall")
	len += format(motd[len], charsmax(motd) - len,"<center><h4><font color=^"white^"><B>-----------------------</B> </font></h4></center>")    
    
    // Motd
	show_motd(id, motd, "[NF] Anti divilgacao") 
} 
