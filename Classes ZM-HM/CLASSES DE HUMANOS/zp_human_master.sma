...
#define CLASS_ACESS ADMIN_LEVEL_F

new const hclass1_name[] = { "Master" }
new const hclass1_info[] = { "Ganha uma Teco TR" }
...
new g_classemasterID

public plugin_precache()
{
	register_plugin("[ZP] Class: Human Master", "1.0", "TEIXEIRA")

	g_classemasterID = zp_register_human_class(hclass1_name, hclass1_info, hclass1_health, hclass1_armor, hclass1_speed, hclass1_gravity)
}

public client_putinserver(id)
{
	if(get_user_flags(id) & CLASS_ACESS)
		zp_set_user_human_class(id, g_classemasterID)
}
...
public zp_human_class_choosed_pre(id, classid) {
	if(g_classemasterID != classid)
		return PLUGIN_CONTINUE;

	//zp_menu_textadd("\r[master]"); // Adiciona um textin igual no 5.0
	if(!(get_user_flags(id) & CLASS_ACESS))
...
public zp_weapon_selected_post(id, wpn_type)
{
	...
	if(zp_get_next_human_class(id) != g_classemasterID || zp_get_user_zombie(id) || zp_get_human_special_class(id))
		return;
