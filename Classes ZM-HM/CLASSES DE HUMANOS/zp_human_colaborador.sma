...
#define CLASS_ACESS ADMIN_LEVEL_D

new const hclass1_name[] = { "Colaborador" }
new const hclass1_info[] = { "Kit Extra" }
const hclass1_health = 250 // 0 - For use default health (Cvar: zp_human_health)
const hclass1_armor = 500
...
new g_classecolaboradorID

public plugin_precache()
{
	register_plugin("[ZP] Class: Human Colaborador", "1.0", "TEIXEIRA")

	g_classecolaboradorID = zp_register_human_class(hclass1_name, hclass1_info, hclass1_health, hclass1_armor, hclass1_speed, hclass1_gravity)
}

public client_putinserver(id)
{
	if(get_user_flags(id) & CLASS_ACESS)
		zp_set_user_human_class(id, g_classecolaboradorID)
}	
...
public zp_human_class_choosed_pre(id, classid) {
	if(g_classecolaboradorID != classid)
		return PLUGIN_CONTINUE;

	//zp_menu_textadd("\r[supremo]"); // Adiciona um textin igual no 5.0
	if(!(get_user_flags(id) & CLASS_ACESS))
...
public zp_weapon_selected_post(id, wpn_type)
{
	...
	if(zp_get_next_human_class(id) != g_classecolaboradorID || zp_get_user_zombie(id) || zp_get_human_special_class(id))
		return;

	zp_give_item(id, "weapon_xm1014", 1)
