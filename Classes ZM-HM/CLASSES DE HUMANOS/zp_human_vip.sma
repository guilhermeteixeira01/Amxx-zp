...
#define CLASS_ACESS ADMIN_LEVEL_H

new const hclass1_name[] = { "Vip" }
new const hclass1_info[] = { "Ganha uma MP5" }
...
{
	register_plugin("[ZP] Class: Human vip", "1.0", "TEIXEIRA")

	g_classevipID = zp_register_human_class(hclass1_name, hclass1_info, hclass1_health, hclass1_armor, hclass1_speed, hclass1_gravity)
}

public client_putinserver(id)
{
	if(get_user_flags(id) & CLASS_ACESS)
		zp_set_user_human_class(id, g_classevipID)
}
...
public zp_weapon_selected_post(id, wpn_type)
{
	...
	if(zp_get_next_human_class(id) != g_classevipID || zp_get_user_zombie(id) || zp_get_human_special_class(id))
		return;
