...
#define CLASS_ACESS ADMIN_LEVEL_B

new const hclass1_name[] = { "Staff" }
new const hclass1_info[] = { "Kit Staff" }
...
{
	register_plugin("[ZP] Class: Human staff", "1.0", "TEIXEIRA")

	g_classestaffID = zp_register_human_class(hclass1_name, hclass1_info, hclass1_health, hclass1_armor, hclass1_speed, hclass1_gravity)
}

public client_putinserver(id)
{
	if(get_user_flags(id) & CLASS_ACESS)
		zp_set_user_human_class(id, g_classestaffID)
}
...
public zp_human_class_choosed_pre(id, classid) {
	if(g_classestaffID != classid)
		return PLUGIN_CONTINUE;

	//zp_menu_textadd("\r[staff]"); // Adiciona um textin igual no 5.0
	if(!(get_user_flags(id) & CLASS_ACESS))
...
public zp_weapon_selected_post(id, wpn_type)
{
	...
	if(zp_get_next_human_class(id) != g_classestaffID || zp_get_user_zombie(id) || zp_get_human_special_class(id))
		return;
