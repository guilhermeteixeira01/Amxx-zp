...
#define PLUGIN "[ZP] Zombie JackFrost"
#define VERSION "0.0"
...
new const zclass_name[ ] = "Zombie JackFrost"
new const zclass_info[ ] = "Anti congelamento"
new const zclass_model[ ] = "jack_frost"
new const zclass_clawmodel[ ] = "v_jackfrost.mdl"
...
public plugin_init()
{
    ...
    //Register Class
    #if !defined TestingTime
    g_jack = zp_register_zombie_class(zclass_name, zclass_info, zclass_model, zclass_clawmodel, zclass_health, zclass_speed, zclass_gravity, zclass_knockback)
    #endif
    //Ham
...
public use_ability(id)
{
    ...
    if(zp_get_user_zombie_class(id)!=g_jack)return
    #endif
...
public client_PreThink(id)
{
    ...
    if(UpdateHud[id-1]<gametime)
    {
        ...
        formatex(Text0, 99, "Pressione [E] para alterar a habilidade^nPressione [F] (Impulso 100) ou [G] (Drop) para usar a habilidade^n^n")
        switch(Selected_Ability[id-1])
        {
            case 0:formatex(Text1, 55, "Habilidade Selecionada: Jogue FireSpere^n")
            case 1:formatex(Text1, 55, "Habilidade Selecionada: Jogue FrostSphere^n")
        }
        if(!Can_Use_Ability[id-1])
        {
            if(Ability_CD[id-1]-gametime>0.0)formatex(Text2, 55, "tempo de recarga: %..1f^n",Ability_CD[id-1]-gametime)
            else formatex(Text2, 55, "Abilidade recaregada: Pronto para usar^n"),Can_Use_Ability[id-1]=true
        }
